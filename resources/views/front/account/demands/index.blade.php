        ...
        <div class="mdc-data-table border-0 w-100 mt-3">
            <table class="mdc-data-table__table" aria-label="Dessert calories">
                ...
                <tr class="mdc-data-table__header-row">
                    ...
                    <th class="mdc-data-table__header-cell">Budget</th>
                ...
                <tbody class="mdc-data-table__content">
                    @foreach($demands as $demand)
                        <tr class="mdc-data-table__row">
                            <td class="mdc-data-table__cell">{{ $demand->id }}</td>
                            <td class="mdc-data-table__cell">{{ $demand->title }}</td>
                            <td class="mdc-data-table__cell">{{ $demand->budget . ' DH' }}</td>
                            <td class="mdc-data-table__cell">{{ $demand->phone_number }}</td>
                            <td class="mdc-data-table__cell">
                                <form action="{{ route('front.account.demands.delete', $demand) }}" method="post">
                                    @csrf
                                    @method('DELETE')
